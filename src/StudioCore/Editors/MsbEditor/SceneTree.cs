...
public class SceneTree : IActionEventHandler
{
    ...
    private unsafe void MapObjectSelectable(Entity e, bool visicon, bool hierarchial = false)
    {
        ...
        {
            _mapEnt_ImGuiID++;
            var name = e.PrettyName;
            var aliasedName = GetModelAliasName(name, e);
        ...
        if (visicon)
        {
            ...
            ImGui.SameLine();
            ImGui.SetCursorPosX(ImGui.GetWindowContentRegionMax().X - 18.0f * DSMapStudio.GetUIScale());
    ...
    public unsafe void OnGui()
    {
        ...
        if (ImGui.Begin(titleString))
        {
            ...
            if (DSMapStudio.LowRequirementsMode)
            {
                ImGui.NewLine();
