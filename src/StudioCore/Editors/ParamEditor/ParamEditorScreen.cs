        ...
        if (ImGui.BeginMenu("Edit"))
        {
            ...
            if (ImGui.BeginMenu("Mass Edit Script"))
            {
                if (ImGui.Selectable("Open Scripts Folder"))
                    Process.Start("explorer.exe", ParamAssetLocator.GetMassEditScriptCommonDir());
                    ...
                    else
                    {
                                ...
                                out var folder))
                        {
                            ...
                            if (PlatformUtils.Instance.OpenFileDialog(
                                    "Select file containing remaining, non-loose params", allParamTypes, out var path))
                            {
                                PlatformUtils.Instance.MessageBox(
        ...
        if (initcmd != null)
            if (initcmd[0] == "select" || initcmd[0] == "view")
                if (initcmd.Length > 2 && ParamBank.PrimaryBank.Params.ContainsKey(initcmd[2]))
                {
                    ...
                    if (initcmd.Length > 3)
                    {
                        var onlyAddToSelection = initcmd.Length > 4 && initcmd[4] == "addOnly";
